...
class LateFuseClassifyTrainer(BaseTrainer):
	def __init__(self, arg, G, torch_device, class_loss):
		super(LateFuseClassifyTrainer, self).__init__(arg, torch_device)
	...
	def train(self, train_loader, val_loader = None):
		...
		for epoch in range(self.start_epoch, self.epoch):
			for i, (input_, target_, clinic_, _) in enumerate(train_loader):
				self.G.train()
				input_, target_, clinic_ = input_.to(self.torch_device), target_.to(self.torch_device), clinic_.to(self.torch_device)
				output_ = self.G(input_, clinic_)
	...
	def forward_for_test(self, input_, target_, clinic_):
		input_  = input_.to(self.torch_device)
		clinic_ = clinic_.to(self.torch_device)
		output_ = self.G(input_, clinic_)
		target_ = target_.to(self.torch_device)
		return input_, clinic_, output_, target_
	...
	def valid(self, epoch, val_loader):
		...
		with torch.no_grad():
			...
			for i, (input_, target_, clinic_, _) in enumerate(val_loader):
				input_ = input_.to(self.torch_device)
				output_ = self.G(input_, clinic_)
				target_ = target_.to(self.torch_device)
	...
	def get_best_th(self, loader):
		y_true = np.array([])
		y_pred = np.array([])
		for i, (input_, target_, clinic_, _) in enumerate(loader):
			input_, clinic_, output_, target_ = self.forward_for_test(input_, target_, clinic_)
	...
	def test(self, test_loader, val_loader):
		...
		with torch.no_grad():
			...
			for i, (input_, target_, clinic_, f_name) in enumerate(test_loader):
				input_, clinic_, output_, target_ = self.forward_for_test(input_, target_, clinic_)
				ground_truth = target_.int().squeeze(1)
				prediction = torch.argmax(output_, dim=1).int()
